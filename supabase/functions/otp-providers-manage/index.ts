...
import { encryptKey, decryptKey } from "../_shared/ai-crypto.ts";
...
async function encryptConfig(cfg: Record<string, unknown>): Promise<Record<string, unknown>> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create") {
      ...
      const cfg = (body?.config_json || {}) as Record<string, unknown>;
