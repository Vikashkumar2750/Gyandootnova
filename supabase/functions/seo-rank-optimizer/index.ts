...
//
// Single-post mode: { post_id, keyword? }
//
...
async function gscQueryPageRows(lovableKey: string, gscKey: string): Promise<any[]> {
  const end = new Date();
...
function pickTargets(rows: any[]): Target[] {
...
async function optimizeWithAI(post: any, keyword: string, position: number) {
...
async function optimizeOne(sb: any, post: any, target: Target) {
  ...
  await sb.from("posts").update({
    ...
    self_check: {
      ...
      last_rank_optimization: {
        ...
        impressions_28d: target.impressions,
  ...
  try {
    ...
    if (targets.length === 0) {
      ...
      targets = (posts || [])
        ...
        .filter((t: Target) => t.position > MIN_POSITION);
    }
