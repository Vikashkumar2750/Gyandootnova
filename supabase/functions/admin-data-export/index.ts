...
function mysqlValue(v: unknown, c: Col): string {
  if (v === null || v === undefined) return "NULL";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const tables: string[] = (schemaInfo as any)?.tables ?? [];
    const ddl: string = (schemaInfo as any)?.ddl ?? "";

    let colsByTable: Record<string, Col[]> = {};
    if (isMySQL) {
          ...
          for (;;) {
            ...
            for (const row of data) {
              if (isMySQL) {
                ...
                const vals = cols.map((c) => mysqlValue((row as any)[c.column_name], c)).join(", ");
                push(`INSERT INTO \`${table}\` (${names}) VALUES (${vals});\n`);
              } else {
                const cols = Object.keys(row);
                const vals = cols.map((c) => qPg((row as any)[c]));
                push(
