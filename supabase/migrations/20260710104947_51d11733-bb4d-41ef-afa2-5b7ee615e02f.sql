...
ALTER TABLE public.coupons ADD COLUMN IF NOT EXISTS repurchase_only boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  -- Repurchase-only: require at least one prior completed purchase by this user
  IF coupon.repurchase_only THEN
    SELECT COUNT(*) INTO prior_purchases
      FROM public.purchases
      WHERE user_id = auth.uid() AND status = 'completed';
    IF prior_purchases < 1 THEN
      RETURN jsonb_build_object('valid', false, 'error', 'This coupon is only valid for returning customers (requires a previous purchase).');
    END IF;
  END IF;
