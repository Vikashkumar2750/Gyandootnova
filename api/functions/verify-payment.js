...
import { normalizeCurrency, paypalAmount } from "../lib/currency.js";
import { PAYPAL_API_BASE } from "../lib/paypal.js";
  ...
  try {
    ...
    if (type === "purchase") {
      ...
      if (purchase?.claim_token) {
        (globalThis).__claimUrl = `https://gyandootnova.in/claim/${purchase.claim_token}`;
      } else {
        (globalThis).__claimUrl = "";
      }
    } else {
      ...
      ? `
        <div style="font-family: 'Georgia', serif; max-width: 600px; margin: 0 auto; padding: 30px; background: #fff;">
          ...
          <div style="text-align: center; margin-top: 25px;">
            <a href="${(globalThis).__claimUrl || "https://gyandootnova.in/dashboard"}" style="background: #B71C1C; color: white; text-decoration: none; padding: 12px 30px; border-radius: 6px; font-weight: bold;">Access Your Book</a>
          </div>
...
async function verifyRazorpay(
  supabase, user, orderId, paymentId, signature, type) {
...
async function verifyPaypal(supabase, user, orderId, type) {
