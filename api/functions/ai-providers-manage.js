...
import { encryptKey, decryptKey, last4 } from "../lib/ai-crypto.js";
...
async function withTimeout(p, ms) {
  return await Promise.race([p, new Promise((_, r) => setTimeout(() => r(new Error("timeout")), ms))]);
}
...
const handler = async (req) => {
  ...
  try {
    ...
    const preAction = ((await req.clone().json().catch(() => ({}))))?.action  | undefined;
    ...
    if (!readOnly) {
      ...
      if (!otpOk) {
        ...
        return new Response(JSON.stringify({ error: "Admin 2FA verification required" }), { status: 403, headers });
      }
    }
    ...
    if (action === "save") {
      ...
      if (!key || key.length < 8) return new Response(JSON.stringify({ error: "invalid api key" }), { status: 400, headers });
      ...
      await audit(admin, { provider, action: "key_updated", admin_user_id: user.id, admin_email: user.email, ip, ua, status: "success", details });
      return new Response(JSON.stringify({ success: true, last4: l4 }), { headers });
    }
    ...
    if (action === "delete") {
      await admin.from("ai_provider_settings").update({
        encrypted_key, key_last4, connection_status,
        last_tested_at, remaining_credits, health_status: "unknown",
        updated_at: new Date().toISOString(), updated_by: user.id,
    ...
    if (action === "test") {
      ...
      if (!key) {
        ...
        if (!row?.encrypted_key) return new Response(JSON.stringify({ error: "no key configured" }), { status: 400, headers });
      ...
      await audit(admin, { provider, action: "connection_tested", admin_user_id: user.id, admin_email: user.email, ip, ua, status: result.status, details });
