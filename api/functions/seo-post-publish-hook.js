...
async function submitToGsc(articleUrl) {
...
async function generateCaptions(post) {
  try {
    const supabaseUrl = process.env.SUPABASE_URL;
    const svc = process.env.SUPABASE_SERVICE_ROLE_KEY;
    const r = await fetch(`${supabaseUrl}/functions/v1/seo-social-captions`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${svc}`, apikey: svc, "x-cron-secret": process.env.SEO_CRON_TOKEN ?? "" },
...
function buildShareBlock(post, captions) {
...
async function sendReport(post, indexingResult, captions) {
  const lovableKey = process.env.LOVABLE_API_KEY;
