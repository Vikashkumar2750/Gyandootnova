...
const handler = async (req) => {
  ...
  const steps = [];
  const log = (name, info) => { steps.push({ step: name, at: new Date().toISOString(), ...info }); };
  ...
  try {
    ...
    for (const cand of candidates || []) {
      ...
      const bookMatch = Number(cand.relevance_score || 0) >= 70 || cand.source === "high_volume_worldwide" || bookTopics.some((t) =>
        kw.includes(String(t).toLowerCase()) || String(t).toLowerCase().includes(kw));
      if (!bookMatch) continue;
      // uniqueness gate 15%
      const dup = (existingPosts || []).some((p) => {
        const s = Math.max(
