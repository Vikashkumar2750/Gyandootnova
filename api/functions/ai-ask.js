...
import { assertSafePublicHttpsUrl } from "../lib/url-guard.js";
...
async function hashQuestion(question, bookId) {
...
function getRelevantChunks(content, question, maxChars = 6000) {
...
async function callAI(provider, apiKey, model, systemPrompt, userMessage, baseUrl) {
  let url = "";
  let headers = {};
  let body;
  ...
  if (provider === "custom") {
    ...
    body = { contents: [{ parts: [{ text: `${systemPrompt}\n\nUser: ${userMessage}` }] }], generationConfig };
...
async function getAuthenticatedUser(req, supabaseUrl, anonKey) {
    ...
    if (action === "ask") {
      ...
      const apiKey = activeProvider === "lovable" || !settings
        ...
        : (settings.api_key_encrypted?.startsWith("env:")
            ? process.env[settings.api_key_encrypted.slice(4)] || ""
