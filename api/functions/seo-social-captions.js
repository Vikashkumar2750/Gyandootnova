...
const handler = async (req) => {
  ...
  if (!apiKey) return new Response(JSON.stringify({ error: "LOVABLE_API_KEY missing" }), { status: 500, headers });
...
Return STRICT JSON with keys (no markdown, no explanation):
{
  ...
  "featured_image_prompt": "detailed AI image prompt for a serene spiritual-themed cover"
    ...
    return new Response(JSON.stringify({ success: true, captions }), { headers });
  } catch (e) {
