// Multi-provider LLM client — ported from _shared/llm-multi.ts
...
async function openAiShape(url, key, model, messages, json, extraHeaders = {}, timeout = 120000) {
...
export async function callChain(chain, messages, json, resolveKey, log, step, exclude) {
...
export function parseJson(raw) {
  const direct = (s) => { try { return JSON.parse(s); } catch { return null; } };
  let out = direct(raw);
  if (out) return out;
